...
def test_direction(start, end, expected):
...
def test_move(point, direction, distance, expected):
    assert _move(point, direction, distance) == expected


def test_from_points_two_points_is_a_single_straight_segment():
...
def test_from_points_single_corner_produces_three_segments():
...
def test_from_points_radius_clamped_to_half_shorter_adjacent_segment():
...
def test_from_points_zero_radius_collapses_corner_to_point():
